...
import {
  ...
  X, ChevronLeft, ChevronRight, ImageOff, CheckCircle,
    ...
    <PageTransition>
      <div className="page-root min-h-screen">
        ...
        <div className="detail-hero-banner">
          ...
          <div className="detail-hero-blob detail-hero-blob-3" />

          <div className="detail-hero-inner">
            {/* Back button */}
            <button className="detail-back-banner" onClick={() => navigate(-1)}>
            ...
            <div className="detail-hero-badges">
              <span className="detail-type-badge-banner">
                {project.type === "software" ? "💻 Software / App" : "🔧 Physical Model"}
              </span>
              {project.category && (
                <span className="detail-hero-chip">{project.category}</span>
              )}
              {project.tags.map((t) => (
                <span key={t} className="detail-hero-chip">{t}</span>
              ))}
            </div>
            ...
            {/* Uploader row */}
            <div className="detail-hero-uploader">
              <img
                ...
                onError={(e) => {
                  (e.currentTarget as HTMLImageElement).src =
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(project.student)}&background=7c3aed&color=fff&size=44`;
                }}
              />
              <div>
                <span className="detail-hero-student-name">{project.student}</span>
                <div className="detail-hero-meta">
                  <span className="detail-hero-meta-item">{project.semester}</span>
                  <span className="detail-hero-meta-dot">·</span>
                  <span className="detail-hero-meta-item">Uploaded {uploadLabel}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ══════════════════════════════════════
            IMAGE — overlaps hero by 48px
        ══════════════════════════════════════ */}
        <div className="detail-image-overlap-wrap">
          <div className="detail-image-pull">
            {images.length > 0 ? (
              <>
                <div className="detail-featured-img-wrap" onClick={() => openLightbox(featuredImg)}>
                  ...
                  <div className="detail-featured-overlay">
                    <span className="detail-zoom-hint">🔍 View Full Size</span>
                  </div>
                </div>
                {images.length > 1 && (
              ...
              <div className="detail-sidebar-inner">
                ...
                <div className="detail-actions">
                  <button
                    ...
                    onClick={() => navigate("/login")}
                  >
                    <span className="detail-action-rocket">🚀</span>
                    Upload Improved Version
                  </button>
                  <button
                    className={`detail-action-btn-save${saved ? " saved-active" : ""}`}
                    onClick={handleSave}
                  >
                    <Bookmark size={16} fill={saved ? "currentColor" : "none"} />
                    {saved ? "Saved" : "Save Project"}
                  </button>
                  <button className="detail-action-btn-share" onClick={handleShare}>
                    <Share2 size={16} />
                    Share Project
                  </button>
                </div>
                ...
                <div className="detail-uploader-card">
                  <div className="detail-uploader-card-inner">
                    <img
                      src={project.avatarUrl}
                      alt={project.student}
                      ...
                      onError={(e) => {
                        (e.currentTarget as HTMLImageElement).src =
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(project.student)}&background=7c3aed&color=fff&size=48`;
                      }}
                    />
                    <div>
                  ...
                  <button className="btn-view-profile">
                    View Profile
                    <span className="profile-arrow"><ArrowIcon size={14} /></span>
                  </button>
                </div>
                    ...
                    <div className="detail-related-cards">
                      {related.map((p) => (
                        ...
                        >
                          ...
                          <span className="detail-related-arrow">
                            <ArrowIcon size={14} />
                          </span>
                        </div>
                      ))}
